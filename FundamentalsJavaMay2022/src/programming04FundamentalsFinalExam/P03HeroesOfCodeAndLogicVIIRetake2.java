...
public class P03HeroesOfCodeAndLogicVIIRetake2 {
    public static void main(String[] args) {
            ...
            switch (command) {
                case "CastSpell":
                    ...
                    if(currentManna >= mannaNeeded) {
                        ...
                        System.out.printf("%s has successfully cast %s and now has %d MP!%n", heroName, spell, currentManna);
                ...
                case "Recharge":
                    ...
                    int chargeDiff = 0;
                    int currentMannaNow = heroesMap.get(heroName).get(1);
                    int newManna = currentMannaNow + rechargeAmount;
                    if(newManna > 200) {
                        chargeDiff = 200 - currentMannaNow;
                        newManna = 200;
                    } else {
                        chargeDiff = rechargeAmount;
                    }
                    heroesMap.get(heroName).set(1, newManna);
                    System.out.printf("%s recharged for %d MP!%n", heroName, chargeDiff);
                    break;

                case "Heal":
                    int healingHealth = Integer.parseInt(tokens[2]);
                    int healingDiff = 0;
                    int currentHealthNow = heroesMap.get(heroName).get(0);
                    int newHealth = currentHealthNow + healingHealth;
                    if(newHealth > 100) {
                        healingDiff = 100 - currentHealthNow;
                        newHealth = 100;
                    } else {
        ...
        heroesMap.forEach((key, value) -> System.out.printf("%s%n" +
                "  HP: %d%n" +
